...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		if(data) {
			...
			else if(user.roles.cache.has(manRole)) {data = await client.newRegister(true, 'MAN', message.guild.id, user.id, message.author.id, Date.now(), [{ name: user.displayName, staffID: message.author.id, date: Date.now() }], { role: manRole, vip: true, vipRole: vipRole, vipStaff: message.author.id, vipDate: Date.now() });}
