...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const age16 = message.guild.members.cache.filter(member => member.displayName.includes('16'));
		const age17 = message.guild.members.cache.filter(member => member.displayName.includes('17'));
		const age18 = message.guild.members.cache.filter(member => member.displayName.includes('18'));
		const age19 = message.guild.members.cache.filter(member => member.displayName.includes('19'));
		const age20 = message.guild.members.cache.filter(member => member.displayName.includes('20'));
