...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const members = message.guild.members.cache.filter(member => member.roles.cache.filter(role => role.id !== message.guild.id).size == 0);
		...
		if(!args[0]) {
			...
			const role = message.guild.roles.cache.get(unregisterRole);
