...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const register = await registers.findOne({ row: row, guildID: message.guild.id });
		...
		const user = await client.fetchUser(register.userID);
		const staff = await client.fetchUser(register.staffID);
