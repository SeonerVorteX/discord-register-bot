...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const penalPoint = await penalPoints.findOne({ guildID: message.guild.id, userID: user.id });
		const staffDatas = await registers.find({ guildID: message.guild.id });
		...
		const Name = name.toLocaleLowerCase()[0].toUpperCase() + name.toLocaleLowerCase().substring(1);
		...
		if(data) {
			...
			const datas = await registers.find({ completed: true, guildID: message.guild.id, userID: user.id }).sort({ row: -1 });
			...
			else if(user.roles.cache.has(manRole)) {data = await client.newRegister(true, 'MAN', message.guild.id, user.id, message.author.id, Date.now(), [{ name: user.displayName, staffID: message.author.id, date: Date.now() }], { role: manRole });}
