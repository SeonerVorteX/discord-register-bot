...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const chat = message.guild.channels.cache.get(guildChat);
		const channel = message.guild.channels.cache.get(log);
		...
		const security = await client.checkSecurity(user.user, quarantineDateLimit);
		const userPenals = await penals.find({ guildID: message.guild.id, userID: user.id });
		const userPoint = await penalPoints.findOne({ guildID: message.guild.id, userID: user.id });
		...
		user.roles.add(manRole);

		if(otherManRoles.length) user.roles.add(otherManRoles);
		...
		if(data) {
			...
			if(data.nameArray.length) {
				...
				options.role = manRole;
				data = await registers.findOneAndUpdate({ completed: false, guildID: message.guild.id, userID: user.id }, { $set: { gender: 'MAN', completed: true, staffID: message.author.id, date: Date.now(), options: options } });

			}

		}
		else {data = await client.newRegister(false, 'MAN', message.guild.id, user.id, message.author.id, Date.now(), undefined, { role: manRole });}
		...
		if(channel && channel.type == 'text') {
			channel.send(Embed.setFooter(Footer).setDescription(`
